...
10. `router/chat.py` 把事件映射成 SSE 推给前端

可以把它记成：

`Frontend -> /api/chat -> SessionManager -> Planner -> TaskExecutor -> CoordinatorAgent -> SpecialistAgent -> Tool -> Summary -> SSE`
...
很多 UI 逻辑是“镜像后端事件”的，而不是靠统一 schema 自动推导。
...
- 直接消费 SSE
- 有斜杠命令
- 对规划/推理/执行/报告分阶段渲染最完整

如果你在调 SSE 事件或 agent 流程，**优先用这个前端验证**。

### 10.2 `app/`

React Native / Expo 移动端，聊天流和桌面/TUI 的渲染逻辑大体镜像。

### 10.3 `desktop/`

桌面端也是基于同一 SSE 协议，适合可视化调试，但当前从代码结构看，仍是一个相对轻量的壳层。
...
**一个以 FastAPI 为壳、以 SessionManager 为编排器、以 Planner + TaskExecutor + CoordinatorAgent 为主执行链、以 SecurityReActAgent 为底层执行骨架、以多前端 SSE 协议为外部表现层的安全测试系统。**
