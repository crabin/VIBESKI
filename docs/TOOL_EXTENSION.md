...
新工具无需修改 `tools/pentest/security/__init__.py` 即可被 vibeski 发现和调用。
...
- **vibeski.tools.basic**：基础工具，vibeski / supervibeski 均可用
- **vibeski.tools.advanced**：高级工具，仅 supervibeski 可用，需用户确认
...
```bash
# 基础工具，逗号分隔模块路径
export VIBESKI_TOOL_MODULES="tools.web_search,mypackage.tools"

# 高级工具
export VIBESKI_TOOL_MODULES_ADVANCED="mypackage.exploit"
```
...
```bash
export VIBESKI_TOOL_MODULES="tools.web_search"
```
...
可选：`sensitivity = "high"` 表示敏感操作，supervibeski 会要求用户确认。
