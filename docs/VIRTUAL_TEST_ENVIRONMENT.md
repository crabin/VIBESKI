...
本文档面向在 **VMware 中搭建的 Ubuntu 目标机** 进行 vibeski 功能验证与测试。提供可直接使用的 **prompt 示例**，确保各功能能触发对应的推理与工具调用。文档中示例 IP 使用占位符 `192.168.x.x` 或 `192.168.56.10`，**请替换为你实际的目标机 IP**。
...
- 宿主机与目标机需能互相 **ping 通**。
- 在宿主机上运行 vibeski，目标为 VMware 内的 Ubuntu 虚拟机 IP（如 `192.168.56.10`）。
...
**目的**：对本机（运行 vibeski 的宿主机）做安全扫描、查看防御状态。
...
### 3.10 专家模式与敏感操作（SuperVibeski）

**目的**：使用 supervibeski 进行需确认的攻击测试/漏洞利用类操作。

1. 输入：`/agent supervibeski` 切换到专家模式。
