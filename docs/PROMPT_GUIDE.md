...
本程序无参数启动即进入交互模式（`python main.py` 或 `vibeski`），交互模式会占据整个终端。在交互界面内可：
...
无参数启动即进入交互模式（`python main.py` 或 `vibeski`）。在交互界面内可通过斜杠命令或模型/提示配置使用提示词、模板或提示词链，例如使用 `/model` 选择后端后，在对话中或相应设置里指定自定义提示词、模板（如 expert）或链（如 expert,technical）。具体以当前界面提供的命令为准；输入 `/` 后回车可查看全部命令。
