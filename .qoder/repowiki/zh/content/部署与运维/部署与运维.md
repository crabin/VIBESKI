...
- [vibeski.spec](file://vibeski.spec)
...
## 更新摘要
**所做更改**
- 更新了发布自动化流程部分，反映.github/workflows/release.yml的完全重构
- 新增了语义化版本控制和自动化版本号提升的详细说明
- 添加了多平台产物构建和智能工件打包的流程说明
- 更新了发布流程的最佳实践和配置要求
...
## 简介
本文件面向Secbot（原名vibeski）的部署与运维，覆盖容器化部署、开发环境配置、性能优化、监控与故障排除、版本管理与发布流程，以及生产环境的部署建议、备份与灾备策略。文档基于仓库内的安装说明、Docker与Node环境说明、发布与CI流程、后端服务入口与依赖清单等资料整理而成，帮助读者在不同环境中稳定、高效地运行Secbot。
...
- [.github/workflows/release.yml](file://.github/workflows/release.yml#L1-L173)
...
- **语义化版本控制**
  - 通过pyproject.toml中的[tool.semantic_release]配置实现语义化版本管理
  - 支持主分支(main)和beta分支的预发布版本控制
  - 自动化的版本号提升和changelog生成
  - tag格式为v{version}，符合标准语义化版本规范

- **自动化版本号提升**
  - 基于Angular风格的提交消息解析（commit_parser = "angular"）
  - 自动检测breaking changes、features和fixes
  - 在CI中自动创建版本标签和发布说明
  - 支持预发布版本（beta分支）

- **多平台产物构建**
  - 并行构建支持：Linux AMD64、Windows AMD64、macOS ARM64、macOS Intel
  - 使用PyInstaller onedir模式避免单文件过大问题
  - 每个平台独立构建，确保兼容性和稳定性
  - Linux平台启用noarchive选项避免CArchiveWriter错误

- **智能工件打包**
  - 自动生成包含版本信息、平台信息和构建时间的README_RELEASE.md
  - 智能提取对应版本的changelog内容
  - 自动下载所有构建产物并上传到GitHub Releases
  - 支持手动workflow_dispatch触发

- **CI/CD流程**
  - GitHub Actions在ubuntu-latest Runner上执行
  - 使用python-semantic-release插件自动版本管理
  - 依赖uv进行快速包管理，替代传统的requirements.txt
  - 支持手动触发和自动触发两种模式

- **用户侧发布使用**
  - 下载对应平台zip，解压后在vibeski目录配置API Key，运行可执行文件进入交互界面
  - 可选配置：在同目录.env中设置模型与提供商参数
  - 支持DeepSeek、Anthropic、Google等多种模型提供商

**章节来源**
- [docs/RELEASE.md](file://docs/RELEASE.md#L1-L86)
- [.github/workflows/release.yml](file://.github/workflows/release.yml#L1-L173)
- [pyproject.toml](file://pyproject.toml#L105-L122)
- [vibeski.spec](file://vibeski.spec#L1-L118)
- [CHANGELOG.md](file://CHANGELOG.md#L1-L8)
...
## 结论
本文基于仓库现有文档与代码，给出了Secbot在容器化、开发环境、性能优化、运维监控、版本管理与发布、以及生产部署与灾备方面的实践建议。最新的发布自动化流程重构显著提升了版本管理的效率和可靠性，支持语义化版本控制、自动化版本号提升、多平台产物构建和智能工件打包。建议在生产环境中遵循最小权限、最小依赖的原则，结合健康检查与日志监控，确保系统的稳定性与可维护性。
