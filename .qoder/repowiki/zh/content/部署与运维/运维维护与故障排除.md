...
LOG["日志(utils.logger)"]
CFG["配置(vibeski_config.settings)"]
end
TUI --> |"HTTP/SSE"| API
RN --> |"HTTP/SSE"| API
API --> Chat
...
图表来源
- [vibeski_config/__init__.py](file://vibeski_config/__init__.py#L16-L149)
...
- 错误捕获：异常写入vibeski_error.log，冻结模式暂停等待查看。
...
- 文档与规范
  ...
  - 配置与密钥：vibeski_config与utils/config_storage的配置与安全存储约定。
...
## 附录
- 快速命令
  - 启动后端：python -m router.main 或 vibeski-server
