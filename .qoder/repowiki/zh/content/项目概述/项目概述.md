...
## 更新摘要
**所做更改**
- 重新组织项目概述结构，反映README.md的全面重构
- 增强安装指导和API配置说明的详细程度
- 更新架构图和组件分析以体现最新的多智能体协作模式
- 强化知识管理体系的介绍和文档结构
- 完善前端多入口接入的说明
...
## 引言
Secbot（原名 vibeski）是一个由人工智能驱动的自动化安全测试机器人平台，旨在为授权环境下的安全测试与主动防御提供一体化解决方案。项目通过事件驱动的多智能体协作、结构化规划与执行、以及前后端分离的现代化架构，实现了从信息收集、漏洞扫描、自动化攻击链到报告生成的全链路自动化。
...
## 附录
- 命名演进
  - 项目从 vibeski 迁移至 vibeski，保留 vibeski 作为兼容入口，文档与示例逐步迁移为 vibeski
- 快速入口
  - 交互模式：无参数运行 main.py 或 vibeski 命令进入全屏 TUI
  - 后端服务：通过 vibeski-server 或 python -m router.main 启动
...
- [vibeski/cli.py:32-80](file://vibeski/cli.py#L32-L80)
