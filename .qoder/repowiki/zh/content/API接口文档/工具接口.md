...
- 工具发现
  ...
  - 环境变量：通过VIBESKI_TOOL_MODULES与VIBESKI_TOOL_MODULES_ADVANCED指定模块路径，模块需导出TOOLS、*_TOOLS或BaseTool子类。
- 工具加载策略
...
## 故障排查指南
- 工具未被发现
  - 检查entry point组名与环境变量VIBESKI_TOOL_MODULES/VIBESKI_TOOL_MODULES_ADVANCED是否正确；
...
- 注册方式
  ...
  - 方式二：环境变量
    - 设置VIBESKI_TOOL_MODULES或VIBESKI_TOOL_MODULES_ADVANCED，逗号分隔多个模块路径。
- 参数Schema设计
