...
- 配置：vibeski_config（全局设置与数据库路径解析）
...
CFG["vibeski_config.Settings<br/>配置中心"]
end
subgraph "数据库"
...
- [vibeski_config/__init__.py](file://vibeski_config/__init__.py#L1-L250)
...
- 配置持久化：vibeski_config通过SQLite user_configs表保存API Key与模型配置，支持从环境变量与keyring回退。

章节来源
- [core/memory/manager.py](file://core/memory/manager.py#L86-L209)
- [core/memory/database_memory.py](file://core/memory/database_memory.py#L14-L38)
- [database/manager.py](file://database/manager.py#L75-L203)
- [vibeski_config/__init__.py](file://vibeski_config/__init__.py#L46-L121)
...
- 嵌入依赖：VulnDBService通过OllamaEmbeddings生成嵌入；OllamaEmbeddings依赖vibeski_config.settings。
...
EMB --> CFG["vibeski_config.Settings"]
...
- 配置要点：确保DATABASE_URL、OLLAMA相关配置正确；必要时通过vibeski_config的SQLite user_configs表保存敏感配置。

章节来源
- [docs/内存与技能系统.md](file://docs/内存与技能系统.md#L64-L141)
- [vibeski_config/__init__.py](file://vibeski_config/__init__.py#L223-L250)