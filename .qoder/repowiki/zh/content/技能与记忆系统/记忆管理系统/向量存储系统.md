...
## 项目结构
向量存储相关的核心代码位于 core/memory 与 core/vuln_db 两个子模块，配合 utils/embeddings 提供嵌入生成能力，vibeski_config 提供 Ollama 相关配置，uv.lock/uv.toml 描述 sqlite-vec/sqlite-vss 依赖与版本约束。
...
CFG["vibeski_config.Settings<br/>Ollama配置"]
end
subgraph "依赖"
SV["sqlite-vec"]
SSS["sqlite-vss"]
...
- [vibeski_config/__init__.py](file://vibeski_config/__init__.py#L162-L180)
...
- vibeski_config.Settings：提供 Ollama 基础地址与模型配置。
...
## 架构总览
向量存储系统围绕 SQLite 展开，通过 sqlite-vec/sqlite-vss 提供 ANN 搜索能力；当缺失 ANN 函数时，自动降级为纯量余弦相似度计算。嵌入生成由 Ollama 提供，配置通过 vibeski_config.Settings 注入。VulnVectorStore 在 SQLiteVectorStore 之上提供漏洞领域的元数据与检索封装。
...
- vibeski_config.Settings：提供 ollama_base_url、ollama_embedding_model 等配置项。
...
- Ollama：通过 utils/embeddings 与 vibeski_config 配置对接，生成与向量存储维度一致的嵌入。
- 内存与数据库记忆：core/memory 提供三层记忆与数据库记忆封装，向量存储可作为长期记忆的补充。

```mermaid
graph TB
EMB["OllamaEmbeddings"] --> CFG["vibeski_config.Settings"]
EMB --> OLL["Ollama 服务"]
...
- [vibeski_config/__init__.py](file://vibeski_config/__init__.py#L169-L173)
...
- 数据库路径
  - DATABASE_URL：SQLite 数据库 URL，默认 sqlite:///./data/vibeski.db。
- 其他
  - LOG_LEVEL、LOG_FILE 等日志配置。

**章节来源**
- [vibeski_config/__init__.py](file://vibeski_config/__init__.py#L162-L180)
- [vibeski_config/__init__.py](file://vibeski_config/__init__.py#L224-L234)
