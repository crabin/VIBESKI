...
- 前置准备：启动Secbot（uv run vibeski 或 python main.py）；选择Ollama或DeepSeek等模型；可选对目标进行授权（编辑data/authorizations.json或使用交互命令）。
...
- 可执行文件：使用scripts/build_release.sh通过PyInstaller生成单文件可执行程序（dist/vibeski）。
...
- 快速命令参考
  ...
  - 启动后端：python main.py 或 uv run vibeski
