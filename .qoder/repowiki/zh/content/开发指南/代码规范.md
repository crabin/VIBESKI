...
- 包与模块组织
  - 核心领域：core.agents、core.patterns、core.memory、core.vuln_db、core.attack_chain、router、utils、tools、skills、prompts、database、scanner、defense、payloads、system、controller、crawler、vibeski、vibeski_config。
  - 前端与终端UI：terminal-ui、app/src。
- 依赖与入口
  - 依赖管理：pyproject.toml集中声明，支持可选依赖与脚本入口。
  - CLI入口：vibeski/vibeski命令与main.py全屏TUI入口，支持--backend/--tui分离调试。
- 文档与设计范式
  - docs/design-paradigms下提供可复用的设计范式，涵盖提交规范、ReAct与工具调用、技能/插件系统、会话与事件、CLI与依赖注入。

```mermaid
graph TB
A["后端入口<br/>pyproject.toml 脚本入口"] --> B["CLI 入口<br/>vibeski/cli.py / main.py"]
...
- [vibeski/cli.py](file://vibeski/cli.py#L32-L80)
...
**图表来源**
- [vibeski_config/__init__.py](file://vibeski_config/__init__.py#L16-L250)
...
## 依赖分析
- 依赖管理
  - pyproject.toml集中声明依赖与可选依赖，提供脚本入口（vibeski/vibeski/vibeski-server/vibeski-server）。
- 包结构与导出
...
## 故障排查指南
- CLI错误处理
  - CLI入口与main.py均捕获异常，写入vibeski_error.log并可暂停等待查看；打包运行时双击暂停。
- 事件总线异常
...
- [vibeski/cli.py](file://vibeski/cli.py#L12-L29)
- [utils/event_bus.py](file://utils/event_bus.py#L137-L138)
- [vibeski_config/__init__.py](file://vibeski_config/__init__.py#L127-L137)
...
- 快速参考
  - 后端入口：pyproject.toml脚本入口；CLI入口：vibeski/cli.py与main.py；配置：vibeski_config/settings；事件：utils/event_bus.py；设计范式：docs/design-paradigms/*。
