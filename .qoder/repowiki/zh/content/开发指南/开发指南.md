...
- 多智能体协作：Q&A、Planner、Core（Vibeski）、Summary等角色分工明确，通过上下文与事件总线解耦。
...
### 工具扩展机制
- Entry Point（推荐，第三方包）：在扩展包的pyproject.toml中声明vibeski.tools.basic或vibeski.tools.advanced，指向模块属性或get_tools()。
- 环境变量（临时扩展）：VIBESKI_TOOL_MODULES/VIBESKI_TOOL_MODULES_ADVANCED，模块需导出TOOLS或*_TOOLS，或提供get_tools()，或导出*Tool类自动实例化。
...
Dev->>Env : "设置 VIBESKI_TOOL_MODULES"
Env->>Reg : "发现模块与工具"
Reg-->>Agent : "提供工具清单"
...
- 错误日志：main.py捕获异常并写入vibeski_error.log，打包运行时暂停以便查看；UI与后端通过EventBus/事件总线上报错误。
