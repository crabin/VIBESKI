...
### 工具注册机制与扩展
- 扩展方式
  - 入口点：在扩展包的配置中声明 vibeski.tools.basic 或 vibeski.tools.advanced，指向模块中的工具列表或工厂函数。
  - 环境变量：通过 VIBESKI_TOOL_MODULES 和 VIBESKI_TOOL_MODULES_ADVANCED 动态注入模块路径，模块需满足若干导出约定之一。
- 自动发现策略
