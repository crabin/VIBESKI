...
- 前端与 CLI：app/* React Native 移动端，terminal-ui/* Ink 终端 TUI，vibeski/* CLI 入口
...
- 协调器 Agent：对外暴露 vibeski，内部持有默认 Agent 与专用 Agent，按 Todo 的 agent_hint/resource 路由执行，聚合结果供汇总
...
```mermaid
flowchart TD
CLI["CLI 入口<br/>vibeski/cli.py"] --> Launch["启动器<br/>vibeski/launch_tui.py"]
...
- [vibeski/launch_tui.py](file://vibeski/launch_tui.py#L291-L342)
...
章节来源
- [vibeski/launch_tui.py](file://vibeski/launch_tui.py#L47-L98)
- [vibeski/launch_tui.py](file://vibeski/launch_tui.py#L270-L288)
