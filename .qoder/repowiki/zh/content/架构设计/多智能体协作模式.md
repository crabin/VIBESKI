...
- 执行层：Core Agent（vibeski/supervibeski）基于ReAct引擎执行工具调用；CoordinatorAgent作为A2A入口，将单步任务路由到专用子Agent；TaskExecutor按拓扑分层并发/串行执行。
...
- 职责
  ...
  - 在普通对话/同步模式下委托默认VibeskiAgent；在分层执行模式下将单步Todo路由到专用子Agent。
