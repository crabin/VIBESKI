...
### 协调器与工具调用机制
- 协调器路由：根据 Todo.agent_hint/resource/tool_hint 选择专用子智能体；无法匹配则回退到默认 Vibeski。
...
- 配置管理：vibeski_config 提供多后端配置、API Key 与 Base URL 管理，支持运行时切换模型。
...
- 工具执行失败：查看工具返回的 ToolResult.error；确认工具参数格式与权限；必要时在 SuperVibeski 模式下进行人工确认。
...
- [vibeski_config/__init__.py](file://vibeski_config/__init__.py#L176-L180)
...
- 使用案例与代码示例路径（不直接展示代码）：
  ...
  - 事件与摘要：参考 SecurityReAct 事件发射与 SummaryAgent 报告生成。
    - [core/patterns/security_react.py](file://core/patterns/security_react.py#L227-L278)
    - [core/agents/summary_agent.py](file://core/agents/summary_agent.py#L111-L183)
  - 配置与模型：参考 vibeski_config 的多厂商 LLM 配置。
    - [vibeski_config/__init__.py](file://vibeski_config/__init__.py#L162-L250)