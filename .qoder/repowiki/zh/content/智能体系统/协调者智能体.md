...
- 在普通对话/同步模式下，兼容历史行为，直接委托给默认的 VibeskiAgent。
...
- 兼容性
  - process() 在普通对话/同步模式下委托给默认 VibeskiAgent，保持向后兼容。
...
FallbackMatch --> |否| FallbackToDefault["回退到默认 VibeskiAgent"]
...
- 无法路由到子智能体
  ...
  - 若均为空，将回退到默认 VibeskiAgent，需确认 Planner 的元数据推断逻辑。
- 工具执行失败
