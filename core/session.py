...
编排 路由 -> Q&A / Planner -> Core Agent (vibeski/supervibeski) -> Summary 的完整流程
管理会话生命周期和事件分发
"""
...
class SessionManager:
    ...
    def new_session(self, agent_type: str = "vibeski", name: str = "") -> Session:
        """创建新会话"""
    ...
    async def handle_message(
        ...
        if stripped.startswith("/"):
            ...
            if cmd in ("/help", "/h", "/?"):
                help_text = (
                    "我是 Vibeski / Secbot 内置的自动化安全测试助手。\n\n"
                    "【角色定位】\n"
                    "- 核心身份：自动化渗透测试与主动安全巡检系统（vibeski 自动模式 / supervibeski 专家模式）。\n"
                    ...
                    "- 核心 Agent（vibeski / supervibeski）基于 ReAct 模式调用安全工具，并通过 EventBus 把思考过程/工具调用结果推送给前端。\n"
